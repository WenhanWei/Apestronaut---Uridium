...
public class ExportControlled {
    ...
    private static KeyStoreConf getTrustStoreConf(PropertySet propertySet, PropertyKey keyStoreUrlPropertyKey, PropertyKey keyStorePasswordPropertyKey,
        ...
        if (required && StringUtils.isNullOrEmpty(trustStoreUrl)) {
            throw new CJCommunicationsException("No truststore provided to verify the GameServer.Server certificate.");
        }
            ...
            if (this.verifyServerCert) {
                ...
                if (this.hostName != null) {
                    ...
                    if (!this.hostName.equalsIgnoreCase(cn)) {
                        throw new CertificateException("GameServer.Server certificate identity check failed. The certificate Common Name '" + cn
                    ...
                    switch (res.getStatus()) {
                        ...
                        case BUFFER_UNDERFLOW:
                            ...
                            if (read(channel, peerNetData) < 0) {
                                throw new CJCommunicationsException("GameServer.Server does not provide enough data to proceed with SSL handshake.");
                            }
                            break;
                        case CLOSED:
                            throw new CJCommunicationsException("Unacceptable SSLEngine result: " + res);
