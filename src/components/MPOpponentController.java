 ...
 * This class controls the opponent character in the multiplayer mode
 */

public class MPOpponentController extends PlayerController {
    ...
    static {
        try {
            receiveSocket = new DatagramSocket(Engine.Instance.my_port1);
    ...
    public static String COMPONENT_NAME = "MPOpponentController";
    ...
    public void Update() {
        ...
        service.execute(new UDPReceive(receiveSocket,Engine.Instance.my_port1));
        if(command == null){
        ...
        if(command.toString().equals("Next Level")){
            ...
            System.out.println("Should show the menu");
...
//            System.out.println("Time elapsed: "+numSeconds + " in Opponent");
        ...
        if (command.toString().equals("Kick or Squash") && !frozen) {
            if (!hasKickCooldown) {
                ...
                if(kick()){
                    System.out.println("PLayer 2: Kicked");
                }
            }
            squash(gameObject);
        }
...
//
//            } else {
//                if (!(progressNextLevel(totalScore,Engine.Instance.currentMultiplayerLevelNum) && health > 0)) {
//                   exit=true;
//                }
//            }
