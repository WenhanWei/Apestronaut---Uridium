...
import components.Network.UDPReceive2;
 ...
 * This class controls the opponent character in the multiplayer mode for the 3-rd player
 */
public class MPOpponentController3Players extends PlayerController {
    ...
    static {
        try {
            receiveSocket = new DatagramSocket(Engine.Instance.my_port2);
    ...
    public static String COMPONENT_NAME = "MPOpponentController3";

    @Override
    public void Update() {
        if(Engine.Instance.player3_exit){
            Engine.Instance.RemoveObject(gameObject);
        }
        service.execute(new UDPReceive2(receiveSocket,Engine.Instance.my_port2));
        ...
        if(command.toString().equals("Exit")){
            ...
            paused=false;
            Time.timeScale = 1f;
            Engine.Instance.player3_exit = true;
            Engine.Instance.RemoveObject(gameObject);
        }
...
//            System.out.println("Time elapsed: "+numSeconds + " in Opponent");
        ...
        if (command.toString().equals("Kick or Squash") && !frozen) {
            if (!hasKickCooldown) {
                ...
                if(kick()){
                    System.out.println("PLayer 3: Kicked");
                }
            }
            squash(gameObject);
        }
